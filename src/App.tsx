...
export default function App() {
  const [user, setUser] = useState<any>({ uid: 'guest-user', displayName: '访客', email: 'guest@example.com' });
  ...
  useEffect(() => {
    ...
    const savedHistory = localStorage.getItem('skingpt_history');
    if (savedHistory) {
      try {
  ...
  const saveToHistory = async (result: SkinAnalysisResult) => {
    ...
    localStorage.setItem('skingpt_history', JSON.stringify(updatedHistory));
  };
  ...
  const Home = () => {
    ...
    return (
      <div className="flex flex-col gap-6 p-6 min-h-screen bg-bg-gray pb-safe">
        <header className="pt-10 pb-6 flex justify-between items-start">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <div className="px-2 py-0.5 bg-slate-200 text-slate-500 text-[10px] font-bold rounded uppercase tracking-wider">
                Local
              </div>
            </div>
            <motion.p 
            ...
            >
              <Sparkles size={14} className="text-brand" /> 纳米科技驱动的科学护肤
            </motion.p>
          </div>
            ...
            >
              ...
              {trends?.chartData && trends.chartData.length >= 2 ? (
                <div className="pointer-events-none">
                  <TrendSparkline data={trends.chartData} />
  ...
  const TrendSparkline = ({ data }: { data: any[] }) => {
    // Show up to 7 items, calculate max and min bounds for a nice scale
    const sortedData = data.slice(0, 7).reverse();
    const scores = sortedData.map(d => d.overall);
    const minScore = Math.max(0, Math.min(...scores) - 10);
    const maxScore = Math.min(100, Math.max(...scores) + 5);

    return (
      <div className="flex items-end justify-between h-[120px] w-full mt-4 gap-2">
        {sortedData.map((d, i) => {
          const height = Math.max(15, ((d.overall - minScore) / (maxScore - minScore)) * 100);
          const dateObj = new Date(d.date);
          const day = i === sortedData.length - 1 ? '今天' : `${dateObj.getMonth() + 1}/${dateObj.getDate()}`;
          const isLatest = i === sortedData.length - 1;
          
          return (
            <div key={i} className="flex flex-col items-center flex-1 gap-2 h-full justify-end">
              <span className={`text-[11px] font-bold ${isLatest ? 'text-white' : 'text-white/70'}`}>
                {d.overall}
              </span>
              <div className="w-full max-w-[20px] bg-white/10 rounded-full h-[70%] flex items-end justify-center overflow-hidden">
                <motion.div 
                  initial={{ height: 0 }}
                  animate={{ height: `${height}%` }}
                  transition={{ delay: i * 0.1, type: "spring", stiffness: 100 }}
                  className={`w-full rounded-full ${isLatest ? 'bg-white' : 'bg-white/60'}`}
                />
              </div>
              <span className={`text-[10px] ${isLatest ? 'text-white font-bold' : 'text-white/50'}`}>
                {day}
              </span>
            </div>
          );
  ...
  const Ingredients = () => {
    ...
    const handleAnalyze = async (imageData?: string) => {
      ...
      try {
        ...
        if (imageData) {
          // Local storage for ingredients as well if needed, for now just navigate
